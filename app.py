...
# ஆப் அமைப்புகள்
st.set_page_config(page_title="Professional Drik Panchangam", layout="wide")
...
def get_drik_panchang(date_obj):
    # திருக்கணித முறைப்படி 5:30 AM IST (0:00 UTC) கணக்கீடு
    jd = swe.julday(date_obj.year, date_obj.month, date_obj.day, 5.5)
    
    # லாகிரி அயனாம்சம் (Lahiri Ayanamsa) - திருக்கணிதத்தின் அடிப்படை
    swe.set_sid_mode(swe.SIDM_LAHIRI)
    
    # நிலவு மற்றும் சூரியன் நிலைகள் (Nirayana Longitudes)
    ...
    karana_idx = int(((m_pos - s_pos) % 360) / 6) % 11 # கரணம் தோராய கணக்கீடு

    # திருக்கணித முடிப்பு நேரக் கணக்கீடு (Iterative Accuracy)
    def find_boundary(jd_start, current_val, calc_type):
        t = jd_start
        step = 0.005 # 7 நிமிட இடைவெளியில் ஆய்வு
        for _ in range(300):
            m = swe.calc_ut(t, swe.MOON, swe.FLG_SIDEREAL)[0][0]
            s = swe.calc_ut(t, swe.SUN, swe.FLG_SIDEREAL)[0][0]
            if calc_type == "nak":
                val = int(m / (360/27))
            else:
                val = int(((m - s) % 360) / 12)
        ...
        # IST நேரத்திற்கு மாற்றுதல்
        res_dt = datetime.combine(date_obj, datetime.min.time()) + timedelta(days=(t - jd_start) + 0.229)
        return res_dt.strftime("%I:%M %p")

    nak_end = find_boundary(jd, nak_idx, "nak")
    tithi_end = find_boundary(jd, tithi_idx, "tithi")
...
# --- UI ---
st.markdown("<h1 class='header-style'>🔱 அஸ்ட்ரோ கைடு - திருக்கணிதப் பஞ்சாங்கம்</h1>", unsafe_allow_html=True)
...
p = get_drik_panchang(selected_date)
pak_class = "waxing" if p['paksha'] == "வளர்பிறை" else "waning"

table_html = f"""
<table class="panchang-table">
    <tr><th>அங்கம்</th><th>திருக்கணித விபரம் (IST)</th></tr>
    <tr><td>📅 <b>வாரம் & பக்கம்</b></td><td>{p['wara']} | <span class="status-tag {pak_class}">{p['paksha']}</span></td></tr>
    <tr><td>🌙 <b>திதி</b></td><td><b>{p['tithi']}</b> ({p['tithi_end']} வரை), பின் <b>{p['next_tithi']}</b></td></tr>
    <tr><td>⭐ <b>நட்சத்திரம்</b></td><td><b>{p['nak']}</b> ({p['nak_end']} வரை), பின் <b>{p['next_nak']}</b></td></tr>
    <tr><td>♈ <b>ராசி / யோகம்</b></td><td>ராசி: {p['raasi']} | யோகம்: {p['yogam']}</td></tr>
...
st.info("குறிப்பு: இது நவீன திருக்கணித (Drik) முறைப்படி கிரகப் பாதைகளை ஆராய்ந்து கணக்கிடப்பட்ட தரவுகள்.")
