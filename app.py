import streamlit as st
import swisseph as swe
from datetime import datetime, timedelta
import pytz
from astral import LocationInfo
from astral.sun import sun
from timezonefinder import TimezoneFinder

# ---------- 1. ஆப் அமைப்புகள் & மேம்படுத்தப்பட்ட CSS வடிவமைப்பு ----------
st.set_page_config(page_title="AstroGuide திருக்கணிதப் பஞ்சாங்கம்", layout="wide")
IST = pytz.timezone('Asia/Kolkata')

st.markdown("""
    <style>
    /* அழகிய பின்னணி மற்றும் பொதுவான எழுத்துரு */
    .stApp { 
        background-color: #FFF8F0; 
        font-family: 'Georgia', serif; 
    }
    
    /* தலைப்பு வடிவமைப்பு */
    .header-style { 
        color: #8B0000 !important; 
        text-align: center; 
        font-weight: bold; 
        padding: 20px;
        border-bottom: 2px solid #8B0000;
        margin-bottom: 20px;
        font-size: 1.8em;
    }
    
    /* மெரூன் வண்ண பெட்டிகள் */
    .main-box { 
        max-width: 500px; 
        margin: auto; 
        padding: 15px; 
        background: #FFFFFF; 
        border-radius: 12px; 
        border-left: 5px solid #8B0000; 
        box-shadow: 0 4px 15px rgba(0,0,0,0.1); 
        margin-bottom: 15px; 
    }
    
    .meroon-header { 
        background-color: #8B0000; 
        color: white !important; 
        text-align: center; 
        padding: 10px; 
        border-radius: 8px; 
        font-size: 1.1em; 
        font-weight: bold; 
        margin-top: 20px; 
        margin-bottom: 12px;
    }
    
    /* தெளிவான அட்டவணை */
    .panchang-table { 
        width: 100%; 
        border-collapse: collapse; 
        background: white; 
        font-size: 0.9em;
    }
    .panchang-table td { 
        padding: 10px; 
        border-bottom: 1px solid #F0F0F0; 
        color: #1a1a1a !important; 
    }
    .panchang-table b { color: #8B0000; }
    
    /* ராசி கட்டம் */
    .rasi-chart { width: 100%; border-collapse: collapse; border: 2px solid #8B0000; table-layout: fixed; }
    .rasi-chart td { border: 1px solid #8B0000; height: 100px; vertical-align: top; padding: 5px; font-size: 0.75em; background: #fff; }
    .rasi-label { color: #8B0000; font-weight: bold; display: block; border-bottom: 1px solid #eee; margin-bottom: 2px; }
    .planet-text { color: #000; font-weight: 600; display: block; line-height: 1.3; }
    </style>
    """, unsafe_allow_html=True)

# ---------------- 2. லாகின் ----------------
if "logged_in" not in st.session_state:
    st.session_state.logged_in = False

if not st.session_state.logged_in:
    st.markdown("<h1 class='header-style'>🔱 AstroGuide உள்நுழைவு</h1>", unsafe_allow_html=True)
    st.markdown('<div class="main-box" style="text-align:center;">', unsafe_allow_html=True)
    if st.button("பஞ்சாங்கத்தைக் காண உள்ளே செல்க"):
        st.session_state.logged_in = True
        st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)
    st.stop()

# ---------------- 3. தலைப்பு மற்றும் தேர்வுகள் ----------------
districts = {"சென்னை": [13.08, 80.27], "மதுரை": [9.93, 78.12], "திருச்சி": [10.79, 78.70], "கோவை": [11.02, 76.96], "நெல்லை": [8.71, 77.76], "சேலம்": [11.66, 78.15]}

st.markdown("<h1 class='header-style'>🔱 AstroGuide திருக்கணிதப் பஞ்சாங்கம்</h1>", unsafe_allow_html=True)

st.markdown('<div class="main-box">', unsafe_allow_html=True)
c1, c2 = st.columns(2)
with c1:
    s_dist = st.selectbox("ஊர்:", list(districts.keys()))
with c2:
    s_date = st.date_input("தேதி:", datetime.now(IST))
st.markdown('</div>', unsafe_allow_html=True)
lat, lon = districts[s_dist]

# ---------------- 4. ஜோதிடக் கணக்கீடுகள் ----------------
def get_all_astro_data(date_obj, lat, lon):
    swe.set_sid_mode(swe.SIDM_LAHIRI)
    jd_ut = swe.julday(date_obj.year, date_obj.month, date_obj.day, 5.5)

    def get_raw(jd):
        m, _ = swe.calc_ut(jd, 1, swe.FLG_SIDEREAL)
        s_p, _ = swe.calc_ut(jd, 0, swe.FLG_SIDEREAL)
        t = ((m[0]-s_p[0])%360)/12
        n = m[0]/(360/27)
        return m[0], s_p[0], int(t), int(n)

    def find_end_time(jd_base, cur_idx, p_type):
        low, high = 0.0, 1.3
        for _ in range(35):
            mid_v = (low + high) / 2
            res_val = get_raw(jd_base + mid_v)
            lookup = {"t":2, "n":3}[p_type]
            if res_val[lookup] == cur_idx: low = mid_v
            else: high = mid_v
        dt = datetime.combine(date_obj, datetime.min.time()) + timedelta(hours=5.5) + timedelta(days=low)
        return dt.strftime('%I:%M %p')

    m_deg, s_deg, t_n, n_n = get_raw(jd_ut)
    tithis = ["பிரதமை", "துவிதியை", "திருதியை", "சதுர்த்தி", "பஞ்சமி", "சஷ்டி", "சப்தமி", "அஷ்டமி", "நவமி", "தசமி", "ஏகாதசி", "துவாதசி", "திரயோதசி", "சதுர்த்தசி", "பௌர்ணமி", "பிரதமை", "துவிதியை", "திருதியை", "சதுர்த்தி", "பஞ்சமி", "சஷ்டி", "சப்தமி", "அஷ்டமி", "நவமி", "தசமி", "ஏகாதசி", "துவாதசி", "திரயோதசி", "சதுர்த்தசி", "அமாவாசை"]
    naks = ["அஸ்வினி", "பரணி", "கார்த்திகை", "ரோகிணி", "மிருகசீரிடம்", "திருவாதிரை", "புனர்பூசம்", "பூசம்", "ஆயில்யம்", "மகம்", "பூரம்", "உத்திரம்", "அஸ்தம்", "சித்திரை", "சுவாதி", "விசாகம்", "அனுஷம்", "கேட்டை", "மூலம்", "பூராடம்", "உத்திராடம்", "திருவோணம்", "அவிட்டம்", "சதயம்", "பூரட்டாதி", "உத்திரட்டாதி", "ரேவதி"]
    months = ['சித்திரை', 'வைகாசி', 'ஆனி', 'ஆடி', 'ஆவணி', 'புரட்டாசி', 'ஐப்பசி', 'கார்த்திகை', 'மார்கழி', 'தை', 'மாசி', 'பங்குனி']
    
    planet_ids = {0: "சூரியன்", 1: "சந்திரன்", 10: "ராகு"}
    transit = {}
    for pid, name in planet_ids.items():
        pos, _ = swe.calc_ut(jd_ut, pid, swe.FLG_SIDEREAL)
        idx = int(pos[0] / 30)
        if idx not in transit: transit[idx] = []
        transit[idx].append(name)

    return {
        "tamil_date": f"{months[int(s_deg/30)%12]} {int(s_deg%30)+1}",
        "wara": ["திங்கள்", "செவ்வாய்", "புதன்", "வியாழன்", "வெள்ளி", "சனி", "ஞாயிறு"][date_obj.weekday()],
        "tithi": tithis[t_n % 30], "t_e": find_end_time(jd_ut, t_n, "t"),
        "nak": naks[n_n % 27], "n_e": find_end_time(jd_ut, n_n, "n"),
        "moon_deg": round(m_deg % 30, 2), "transit": transit,
        "n_idx": n_n % 27, "t_idx": t_n % 30, "month_name": months[int(s_deg/30)%12]
    }

res = get_all_astro_data(s_date, lat, lon)

# --- முகூர்த்தம் ---
is_muhurtham = "இன்று சுப முஹூர்த்தம் இல்லை"
if res['t_idx'] in [1, 2, 4, 6, 9, 10, 12]:
    is_muhurtham = "✅ இன்று சுப முஹூர்த்த நாள்"

# ---------------- 5. பஞ்சாங்கம் அட்டவணை ----------------
st.markdown(f"""
<div class="main-box">
<table class="panchang-table">
    <tr><td>📅 <b>தமிழ் தேதி</b></td><td><b>{res['tamil_date']}</b></td></tr>
    <tr><td>🌙 <b>திதி</b></td><td><b>{res['tithi']}</b> ({res['t_e']} வரை)</td></tr>
    <tr><td>⭐ <b>நட்சத்திரம்</b></td><td><b>{res['nak']}</b> ({res['n_e']} வரை)</td></tr>
    <tr><td>💎 <b>முஹூர்த்தம்</b></td><td><b>{is_muhurtham}</b></td></tr>
    <tr><td>📊 <b>சந்திர பாகை</b></td><td><b>{res['moon_deg']}°</b></td></tr>
</table>
</div>
""", unsafe_allow_html=True)

# ---------------- 6. ராசி கட்டம் ----------------
st.markdown("<div class='meroon-header'>🎡 இன்றைய கோச்சார ராசி கட்டம்</div>", unsafe_allow_html=True)
def get_p(i): return "<br>".join(res['transit'].get(i, []))

st.markdown(f"""
<div style="max-width:400px; margin:auto;">
<table class="rasi-chart">
    <tr><td><span class='rasi-label'>மீனம்</span>{get_p(11)}</td><td><span class='rasi-label'>மேஷம்</span>{get_p(0)}</td><td><span class='rasi-label'>ரிஷபம்</span>{get_p(1)}</td><td><span class='rasi-label'>மிதுனம்</span>{get_p(2)}</td></tr>
    <tr><td><span class='rasi-label'>கும்பம்</span>{get_p(10)}</td><td colspan="2" rowspan="2" style="background:#fdfdfd; text-align:center; vertical-align:middle; color:#8B0000; font-weight:bold;">ராசி<br>கட்டம்</td><td><span class='rasi-label'>கடகம்</span>{get_p(3)}</td></tr>
    <tr><td><span class='rasi-label'>மகரம்</span>{get_p(9)}</td><td><span class='rasi-label'>சிம்மம்</span>{get_p(4)}</td></tr>
    <tr><td><span class='rasi-label'>தனுசு</span>{get_p(8)}</td><td><span class='rasi-label'>விருச்சி</span>{get_p(7)}</td><td><span class='rasi-label'>துலாம்</span>{get_p(6)}</td><td><span class='rasi-label'>கன்னி</span>{get_p(5)}</td></tr>
</table>
</div>
""", unsafe_allow_html=True)

# ---------------- 7. சந்திராஷ்டமம் ----------------
st.markdown("<div class='meroon-header'>🌙 சந்திராஷ்டமம்</div>", unsafe_allow_html=True)
naks_list = ["அஸ்வினி", "பரணி", "கார்த்திகை", "ரோகிணி", "மிருகசீரிடம்", "திருவாதிரை", "புனர்பூசம்", "பூசம்", "ஆயில்யம்", "மகம்", "பூரம்", "உத்திரம்", "அஸ்தம்", "சித்திரை", "சுவாதி", "விசாகம்", "அனுஷம்", "கேட்டை", "மூலம்", "பூராடம்", "உத்திராடம்", "திருவோணம்", "அவிட்டம்", "சதயம்", "பூரட்டாதி", "உத்திரட்டாதி", "ரேவதி"]
target_idx = (res['n_idx'] - 16) % 27
st.markdown(f"""
<div class='main-box' style='border-left-color:red;'>
    <b style='color:red;'>⚠️ {naks_list[target_idx]}</b> நட்சத்திரத்திற்கு இன்று சந்திராஷ்டமம்.
</div>
""", unsafe_allow_html=True)

# ---------------- 8. விசேஷங்கள் ----------------
st.markdown("<div class='meroon-header'>🗓️ இன்றைய விசேஷங்கள்</div>", unsafe_allow_html=True)
vrat_db = {
    ("அமாவாசை", None, "மார்கழி"): ["🐒", "ஸ்ரீ ஹனுமன் ஜெயந்தி"],
    ("அமாவாசை", None, None): ["🌑", "அமாவாசை தர்ப்பணம்"],
    ("பௌர்ணமி", None, None): ["🌕", "பௌர்ணமி விரதம்"],
    ("சதுர்த்தி", None, None): ["🐘", "சங்கடஹர சதுர்த்தி"],
    ("திரயோதசி", None, None): ["🐂", "பிரதோஷம்"]
}

found = False
for (t, n, m), d in vrat_db.items():
    if t == res['tithi'] and (m is None or m == res['month_name']):
        found = True
        st.markdown(f"""
        <div class="main-box" style="padding: 10px; border-left-color: #2E7D32;">
            <span style="font-size:1.5em;">{d[0]}</span> <b>{d[1]}</b>
        </div>
        """, unsafe_allow_html=True)

if not found:
    st.info("இன்று குறிப்பிட்ட விசேஷங்கள் ஏதுமில்லை.")
