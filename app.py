...
def get_precise_panchang(date_obj, lat, lon):
    ...
    return {
        "m_deg": round(m_start, 2), "tamil": f"{t_month} {t_date}", 
...
st.markdown(f"<div class='special-note'>📅 தமிழ் தேதி: {p['tamil']} | {p['wara']}</div>", unsafe_allow_html=True)

st.markdown(f"""
<table class="panchang-table">
    <tr><th>அங்கம் (Panchangam)</th><th>விவரம் (Details)</th></tr>
    <tr><td>📅 <b>தமிழ் மாதம் & தேதி</b></td><td><b>{p['tamil']}</b></td></tr>
    <tr><td>🌙 <b>திதி சஞ்சாரம்</b></td><td><b>{p['tithi']}</b><br>முடிவு: {p['t_end']}</td></tr>
    <tr><td>⭐ <b>நட்சத்திரம்</b></td><td><b>{p['nak']}</b><br>முடிவு: {p['n_end']}</td></tr>
