...
def get_precise_panchang(date_obj, lat, lon):
    # Integer conversions for date safety
    ...
    def get_raw_astronomy(jd):
        # Planet IDs: 0 for Sun, 1 for Moon
        m_calc, _ = swe.calc_ut(jd, 1, swe.FLG_SIDEREAL)
        s_calc, _ = swe.calc_ut(jd, 0, swe.FLG_SIDEREAL)
        m_deg, s_deg = m_calc[0], s_calc[0]
    ...
    return {
        "m_deg": round(m_start, 2), "wara": wara, "t_month": t_month, "t_date": t_date,
        "sunrise": sunrise, "sunset": sunset,
        "tithi": tithis[t_now % 30], "tithi_end": t_end_dt.strftime("%I:%M %p"), "next_tithi": tithis[(t_now+1)%30],
        "nak": naks[n_now % 27], "nak_end": n_end_dt.strftime("%I:%M %p"), "next_nak": naks[(n_now+1)%27],
...
st.markdown(f"<div class='special-note'>📅 தமிழ் தேதி: {p['t_month']} {p['t_date']} | கிழமை: {p['wara']}</div>", unsafe_allow_html=True)
