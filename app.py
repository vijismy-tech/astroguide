...
# ---------------- Logic ----------------
districts = {"சென்னை": [13.08, 80.27], "மதுரை": [9.93, 78.12], "திருச்சி": [10.79, 78.70], "கோவை": [11.02, 76.96], "நெல்லை": [8.71, 77.76]}
...
def get_full_panchang(date_obj, lat, lon):
    ...
    def get_raw(jd):
        ...
        return int(t), int(n), int(y % 27), int(k)

    def find_end(jd_base, cur_idx, p_type):
        low, high = 0.0, 1.3
        for _ in range(35):
            mid_v = (low + high) / 2
            res = get_raw(jd_base + mid_v)
            lookup = {"t":0, "n":1, "y":2, "k":3}[p_type]
    ...
    return {
        ...
        "tithi": tithis[t_n % 30], "t_e": find_end(jd_ut, t_n, "t"), "t_nx": tithis[(t_n+1)%30],
        "nak": naks[n_n % 27], "n_e": find_end(jd_ut, n_n, "n"), "n_nx": naks[(n_n+1)%27],
        "yoga": yogas[y_n % 27], "y_e": find_end(jd_ut, y_n, "y"),
        "karan": karans[k_n % 11], "k_e": find_end(jd_ut, k_n, "k"),
