...
def get_precise_panchang(date_obj, lat_val, lon_val):
    ...
    def get_raw_astronomy(jd):
        # கிரகங்களின் ஐடிகள்: 1-சந்திரன், 0-சூரியன்
        m_pos, _ = swe.calc_ut(jd, 1, swe.FLG_SIDEREAL)
        s_pos, _ = swe.calc_ut(jd, 0, swe.FLG_SIDEREAL)
    ...
    # --- சூரிய உதயம் & அஸ்தமனம் (எரர் வராதவாறு திருத்தப்பட்டது) ---
    ...
    return {
        ...
        "tamil": f"{t_month} {t_date}", "tithi": tithis[t_now % 30], "tithi_end": t_end_dt.strftime("%d-%m %I:%M %p"),
        "nak": naks[n_now % 27], "nak_end": n_end_dt.strftime("%d-%m %I:%M %p"),
...
<table class="panchang-table">
    ...
    <tr><td>🌙 <b>திதி சஞ்சாரம்</b></td><td><b>{p['tithi']}</b><br><span class='sub-text'>முடிவு: {p['tithi_end']}</span></td></tr>
