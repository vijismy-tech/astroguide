...
# ---------- CSS வடிவமைப்பு (மாற்றங்கள் செய்யப்படவில்லை) ----------
st.markdown("""
    <style>
...
<table class="panchang-table">
    ...
    <tr><td>⭐ <b>நጀመሪያ நட்சத்திரம்</b></td><td><b>{res['nak']}</b> வரை ({res['n_e']})<br><span class='next-info'>அடுத்து: {res['n_nx']}</span></td></tr>
...
# ---------- சந்திராஷ்டமம் அட்டவணை பகுதி (புதிதாக இணைக்கப்பட்டது) ----------
...
try:
    # தற்போதைய மற்றும் அடுத்த நட்சத்திர எண்கள்
    current_nak_idx = naks_list.index(res['nak'])
    next_nak_idx = (current_nak_idx + 1) % 27

    # சந்திராஷ்டம நட்சத்திரங்கள் (17-வது நட்சத்திரம்)
    ...
    <p style='text-align: center; font-size: 0.73em; color: #555
