...
st.set_page_config(page_title="AstroGuide Tamil Panchangam", layout="wide")
...
st.markdown("""
    ...
    .panchang-table td { padding: 10px; border: 1px solid #eee; font-weight: 500; }
    ...
    .planet-text { color: #1a1a1a; font-weight: 800; font-size: 1em; line-height: 1.4; }
    ...
    .center-info-box { text-align: center; background: #FFFBF2; border: 2.5px double #D4AF37; border-radius: 12px; padding: 15px; }
...
# ---------- 4. பஞ்சாங்கக் கணக்கீடுகள் ----------
def get_panchangam(date_obj, lat, lon):
    swe.set_sid_mode(swe.SIDM_LAHIRI)
    # சூரிய உதய நேரம் (Udaya Thithi-க்காக)
    ...
    s_info = sun(observer=city.observer, date=date_obj, tzinfo=pytz.timezone(tz_name))
    sunrise = s_info["sunrise"]
    
    # உதய நேரத்தின் Julian Day
    ...
    def get_raw(jd):
        ...
        thithi_val = ((m[0]-s[0])%360)/12
        nak_val = m[0]/(360/27)
        return int(thithi_val), int(nak_val), m[0], s[0]

    t_idx, n_idx, m_deg, s_deg = get_raw(jd_sunrise)
    
    # தமிழ் வருடம் (ஏப்ரல் 14 மாற்றம்)
    ...
    y_idx = (date_obj.year - 1987) % 60
    if (date_obj.month < 4) or (date_obj.month == 4 and date_obj.day < 14): y_idx -= 1
    ...
    for pid, name in p_map.items():
        pos, _ = swe.calc_ut(jd_sunrise, pid, swe.FLG_SIDEREAL)
        deg = pos[0]; idx = int(deg / 30)
        v = " <span class='vakra-text'>(வ)</span>" if pos[3] < 0 else ""
        p_s = f"<div class='planet-text'>{name}{v} {int(deg%30)}°</div>"
        if idx not in res_pos: res_pos[idx] = []
        res_pos[idx].append(p_s)
        if pid == 10:
            ki = (idx + 6) % 12
    ...
    return {
        "y": years_60[y_idx % 60], "m": months[int(s_deg/30)%12], "d": int(s_deg%30)+1,
        "tithi": tithis[t_idx % 30], "nak": naks[n_idx % 27], "n_idx": n_idx % 27,
        "rise": sunrise.strftime("%I:%M %p"), "set": s_info["sunset"].strftime("%I:%M %p"),
...
res = get_panchangam(s_date, lat, lon)
...
<div class='meroon-header'>⏳ அசுப நேரங்கள்</div>
<table class="panchang-table">
        ...
        <tr><td>{draw_box(10)}</td>
            <td colspan="2" rowspan="2" style="vertical-align:middle;">
                <div class="center-info-box">
                    ...
                    <div style="font-size:0.8em; color:#333;">{s_date.strftime('%d-%m-%Y')}</div>
                </div>
            </td>
            <td>{draw_box(3)}</td></tr>
...
st.markdown(f"<div class='meroon-header'>🌙 இன்றைய சந்திராஷ்டமம்: <b style='color:yellow;'>{naks_list[(res['n_idx']-16)%27]}</b></div>", unsafe_allow_html=True)
