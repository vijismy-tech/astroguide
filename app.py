...
for (t, n, m), d in vrat_db.items():
    if t == res['tithi'] and (n is None or n == res['nak']) and (m is None or m == res['month_name']):
        found_v = True
        st.markdown(f"""<table class="vrat-table"><tr><td style="width:35%; text-align:center;"><img src="{d[2]}" width="90" height="90" style="border-radius:50%; object-fit:cover; border:2px solid #8B0000;"></td><td style="padding:10px;"><b>✨ {d[0]}</b><br><small>{d[1]}</small></td></tr></table>""", unsafe_allow_html=True)
